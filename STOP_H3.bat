@echo off
rem H3 stop launcher (ASCII only). Thin wrapper around: python -X utf8 -m h3app.launcher stop
...
pushd "%APPDIR%"
"%PYEXE%" %PYARG% -X utf8 -m h3app.launcher stop
set "RC=%ERRORLEVEL%"
popd

if not "%RC%"=="0" (
  echo.
  echo H3 stop finished with exit code %RC%.
  pause
)
exit /b %RC%
